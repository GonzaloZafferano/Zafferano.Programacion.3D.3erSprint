        ...
        if($request->getMethod() === 'POST'){
            ...
            isset($parametros['precio'])){
              ...
              if(!empty($parametros['nombre']) && !empty($parametros['perfil']) &&
                  !empty($parametros['precio'])){

                    if(is_numeric($parametros['precio'])){
                        $perfil = $parametros["perfil"];
                       if (strcasecmp($perfil,"cervecero") == 0 ||
                            ...
                            $respuesta = json_encode(array("mensaje" => "ERROR. El perfil solo puede ser #bartender, #cocinero o #cervecero."));
                            $r = $r->withStatus(400);
                        }
                    }else{
                        $respuesta = json_encode(array("mensaje" => "ERROR. El precio debe ser numerico."));
                        $r = $r->withStatus(400);
                    }
              }else{
