...
class ProductoController extends Producto
{
    ...
    public function GenerarArchivoCSV($request, $response, $args){
      try{
       ...
       if(Archivo::Guardar("./Archivos/productos.csv", $productos, function ($item)
        {
          ...
          $payload = json_encode(array("mensaje" => "Error al intentar generar el archivo"));
        }  
        ...
        if ($handle = opendir('./Archivos/')) {
          while (false !== ($entry = readdir($handle))) {
              if ($entry != "." && $entry != "..") {
            echo "<a href='./Archivos/productos.csv'>Descargue su archivo.</a>" . PHP_EOL;
            }
          }
      }
    }
      catch(Exception $ex){
        $payload = json_encode(array("mensaje" => "Ha ocurrido un error: ". $ex->getMessage()));
      }
